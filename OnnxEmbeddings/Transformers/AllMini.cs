...
{
    public class AllMini
    {
        private AllMini()
        {
        }
        private static object locker = new object();
        private static AllMini? _Instance;
        public static AllMini Instance
        {
            get
            {
                lock (locker)
        ...
        public List<(string, float[])> Encode(string[] sentences, EmbeddingModels embeddingType)
        {
            ...
            if (embeddingType != EmbeddingModels.all_MiniLM_L12_v2 && embeddingType != EmbeddingModels.all_MiniLM_L6_v2)
            {
        ...
        public List<(string, float[])> Encode(TokenizationResult inputs, int batch, EmbeddingModels embeddingType)
        {
            List<(string, float[])> embeddings = new List<(string, float[])>();
            ...
            for (int a = 0; a < inputs.input_ids.Length; a+=batch)
            {
                DateTime d1 = DateTime.UtcNow;

                long[][] batch_input_ids =       inputs.input_ids.Skip(aa * batch).Take(batch).ToArray();
                long[][] batch_attention_mask =  inputs.attention_mask.Skip(aa * batch).Take(batch).ToArray();
                long[][] batch_token_type_ids =  inputs.token_type_ids.Skip(aa * batch).Take(batch).ToArray();
                string[] batch_sentences =       inputs.Sentences.Skip(aa* batch).Take(batch).ToArray();
                aa++;
                int[] dims = [inputs.input_ids.Length, inputs.input_ids[0].Length];
                ...
                DateTime d2 = DateTime.UtcNow; double dif = (d2 - d1).TotalMilliseconds; Console.WriteLine("Inference Complete"); Console.WriteLine($"Compute Time {dif.ToString("N0")} Milliseconds");

                for (int i = 0; i < batch; i++)
                {
                    float[] first = sentence_embeddings[i].data<float>().ToArray();
                    string sentence = batch_sentences[i];
